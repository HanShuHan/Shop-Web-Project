	...
	<div class="container border padding-10" id="login-div">
		...
		<div class="tab-content border border-top-0" id="nav-tabContent">
			<div class="tab-pane fade show active padding-10" id="nav-home"
				...
				<form name="cpw" onsubmit="return validate()"
					action="${contextRoot}/cpassword?id=${sessionScope.accountId}"
					method="Post">
					<div class="form-group">
						<input type="password" class="form-control" name="pwd" id="pwd"
							placeholder="請輸入密碼" hidden required="required"
							value="${passWord}" />
					</div>
					<div class="form-group">
						<label>請輸入新密碼</label> <input type="password" class="form-control"
							name="npwd" id="npwd" placeholder="請輸入密碼" onblur="checkNPW()"
							required="required" /> <span id="npwsp" style="font-size: 10px;"></span>
					</div>
					<div class="form-group">
						<label>密碼再確認</label> <input type="password" class="form-control"
							name="pwdcheck" id="pwdcheck" placeholder="請再次確認"
							onblur="checkPWR()" required="required" /> <span id="pwck"
							style="font-size: 10px;"></span>
					</div>
					<br>
					<span id="rsp" style="font-size: 10px;"></span>
					<button type="submit" class="btn btn-primary">確認</button>
					<a href="${contextRoot}/account?id=${sessionScope.accountId}"><button
...
<script>
	function isPwdValid() {
		let pwd = document.getElementById("npwd");
	...
	function validate() {
	  if (isPwdValid() && isRepeatPwdValid()) {	 
		  return true;
	  } else {
	       return false;
		 }

		...
		<div class="container">
			<div class="col-md-12">
				<div class="invoice">
					...
					<div class="invoice-company text-inverse f-w-600">訂單確認</div>
					<!-- end invoice-company -->
					...
					<div class="invoice-header">
						<div class="invoice-from">
							...
							<address class="m-t-5 m-b-5">
								...
								<br>
								<c:out value="${account.mail}"></c:out>
								<br> Phone:
								<c:out value="${account.phone}"></c:out>
							</address>
						</div>
							...
							<table class="table table-invoice">
								<c:forEach items="${OrderDetail}" var="OrderDetail"
									...
									<tbody>
										<tr>
											<td><c:if
													test="${OrderDetail.product.coverImage1 != null}">
													<img width="50" class="image1" alt=""
														src='<c:url value="/showImage1/${OrderDetail.product.productId}"/>'>
												</c:if> <span class="text-inverse">${OrderDetail.product.name}</span><br>
												<small>${OrderDetail.product.detail1}</small></td>
						...
						<c:choose>
							<c:when test="${sessionScope.crs == null}">
								<form action="${contextRoot}/couponck" method="post">
									<input type="text" name="couponck">
									<button type="submit" id="coupon"
										class="btn btn-sm btn-white m-b-10 p-l-5">確認</button>
								</form>
							</c:when>
							<c:otherwise>
									已使用折扣
						...
						<div class="invoice-price">
							<div class="invoice-price-left">
								<div class="invoice-price-row">
									<div class="sub-price">
										<small>金額</small> <span class="text-inverse">NTD$${sum}</span>
									</div>
									...
									<div class="sub-price">
										<small>店到店費用</small> <span class="text-inverse">NTD$100</span>
									</div>
								</div>
							</div>
							<div class="invoice-price-right">
								<c:set var="tp"
									value="${sum*percentage.percentage/10-Fix.fixed+100-cp}"></c:set>
								<fmt:parseNumber var="tp" integerOnly="true" type="number"
									value="${tp}" />
					...
					<div class="invoice-note">
						* 請確認購買內容<br> * 貨運統一酌收100元運費<br> * 請確實填寫收貨人資料
					</div>
					<!-- end invoice-note -->
					...
					<div class="invoice-footer">
						...
						<p class="text-center">
							<span class="m-r-10"><i class="fa fa-fw fa-lg fa-globe"></i>
								lochalhost:8080/A-Jen/index</span> <span class="m-r-10"><i
								class="fa fa-fw fa-lg fa-phone-volume"></i> T:02-6631-6666</span> <span
								class="m-r-10"><i class="fa fa-fw fa-lg fa-envelope"></i>
								eeit38@gmail.com</span>
						</p>
						<form:form action="${contextRoot}/ordercke"
							modelAttribute="OrderBean" method="post"
							enctype="multipart/form-data">
							<form:input type="hidden" value="${OrderBean.merchantTradeNo}"
								path="merchantTradeNo" />
							<form:input type="hidden" value="${OrderBean.merchantTradeDate}"
								path="merchantTradeDate" />
							<form:input type="hidden" value="${OrderBean.itemName}"
								path="itemName" />
							<form:input type="hidden" value="${OrderBean.accountId}"
								path="accountId" />
							<form:input type="hidden" value="${tp}" path="totalAmount" />
							<button class="btn btn-sm btn-white m-b-10 p-l-5" type="submit"
								value="送單">確認結帳</button>
						</form:form>
					</div>
					<!-- end invoice-footer -->
				</div>
